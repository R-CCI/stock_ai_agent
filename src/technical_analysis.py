...
def detect_trend(df: pd.DataFrame, lookback: int = 50) -> dict:
    ...
    if pct_change > 5:
        direction = "Strong Uptrend"   # kept for tab translation logic
    elif pct_change > 1:
            ...
            if head > left and head > right:
                ...
                if shoulder_diff < 0.05:  # shoulders within 5% of each other
                    ...
                    patterns.append({
                        ...
                        "reliability": "Alta" if shoulder_diff < 0.03 else "Moderada",
                    })
        ...
        for i in range(len(local_min_idx) - 2):
            ...
            if head < left and head < right:
                ...
                if shoulder_diff < 0.05:
                    patterns.append({
                        "type": "Cabeza y Hombros Invertido (Alcista)",
                        "head_price": round(float(head), 2),
...
def detect_divergences(df: pd.DataFrame, lookback: int = 50) -> list:
    ...
    # Divergencia alcista: precio hace minimo menor, RSI hace minimo mayor
    if len(local_min_idx) >= 2:
        i, j = local_min_idx[-2], local_min_idx[-1]
        if close.iloc[j] < close.iloc[i] and rsi.iloc[j] > rsi.iloc[i]:
            divergences.append({
                "type": "Divergencia Alcista (Bullish)",
                "description": "El precio hizo un minimo menor mientras el RSI hizo un minimo mayor - posible senal de reversion al alza",
                "date": str(close.index[j].date()),
            })

    # Divergencia bajista: precio hace maximo mayor, RSI hace maximo menor
    if len(local_max_idx) >= 2:
        i, j = local_max_idx[-2], local_max_idx[-1]
        if close.iloc[j] > close.iloc[i] and rsi.iloc[j] < rsi.iloc[i]:
            divergences.append({
                "type": "Divergencia Bajista (Bearish)",
                "description": "El precio hizo un maximo mayor mientras el RSI hizo un maximo menor - posible senal de agotamiento",
                "date": str(close.index[j].date()),
            })
...
def detect_sma_crossovers(df: pd.DataFrame) -> list:
    ...
    for i in range(1, len(recent)):
        ...
        if s50_prev <= s200_prev and s50_now > s200_now:
            crossovers.append({
                "type": "Cruz Dorada - Golden Cross (Alcista)",
                "date": str(idx.date()),
                "description": "SMA50 cruzo por encima de SMA200 - senal alcista de largo plazo",
            })
        elif s50_prev >= s200_prev and s50_now < s200_now:
            crossovers.append({
                "type": "Cruz de la Muerte - Death Cross (Bajista)",
                "date": str(idx.date()),
                "description": "SMA50 cruzo por debajo de SMA200 - senal bajista de largo plazo",
            })
...
def generate_ta_summary(df: pd.DataFrame, ticker: str) -> dict:
    ...
    # Signal assessments — all labels in Spanish for consistent LLM input
    ...
    if rsi_val > 70:
        ...
        rsi_signal = "Momento alcista"
    elif rsi_val < 40:
        rsi_signal = "Momento bajista"
    else:
        rsi_signal = "Neutral"
    ...
    macd_signal = "Alcista" if macd_val > macd_sig else "Bajista"

    adx_val = float(last["ADX"]) if not pd.isna(last["ADX"]) else 0
    if adx_val > 40:
        adx_signal = "Tendencia fuerte"
    elif adx_val > 25:
        adx_signal = "En tendencia"
    else:
        adx_signal = "Sin tendencia clara"
