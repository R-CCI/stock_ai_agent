...
def render_options_tab(
    ...
    if not options_analysis:
        message = raw_options.get("message") or "No hay datos de opciones disponibles para este ticker."
        expirations = raw_options.get("all_expirations", [])
        st.warning(f"⚠️ {message}")
        if expirations:
            st.caption(
                f"Yahoo Finance devolvió {len(expirations)} fecha(s) de expiración, "
                "pero ninguna cadena fue utilizable."
            )
        return
    ...
    if all_expirations:
        ...
        if selected_exp != current_exp:
            with st.spinner(f"Cargando cadena de opciones para {selected_exp}..."):
                ...
                if new_data.get("has_valid_chain"):
                    ...
                    st.warning(new_data.get("message") or f"No se pudo cargar la expiración {selected_exp}.")
