...
def get_valuation_data(ticker: str) -> dict:
    ...
    def _safe_metric_mean(df: pd.DataFrame, column: str):
    ...
    try:
        ...
        try:
            if industry:
                ...
                if ind_df is not None and not ind_df.empty:
                    industry_averages = {
                        "P/E": _safe_metric_mean(ind_df, "P/E"),
                        "PEG": _safe_metric_mean(ind_df, "PEG"),
                        "P/S": _safe_metric_mean(ind_df, "P/S"),
                        "P/B": _safe_metric_mean(ind_df, "P/B"),
                        "P/C": _safe_metric_mean(ind_df, "P/C"),
                        "P/FCF": _safe_metric_mean(ind_df, "P/FCF"),
                    }
        except Exception:
            pass
        ...
        valuation["industry_averages"] = industry_averages
        valuation["industry_pe_avg"] = industry_averages.get("P/E")
        valuation["industry_peg_avg"] = industry_averages.get("PEG")
        valuation["industry_ps_avg"] = industry_averages.get("P/S")
        valuation["industry_pb_avg"] = industry_averages.get("P/B")
        valuation["industry_pc_avg"] = industry_averages.get("P/C")
        valuation["industry_pfcf_avg"] = industry_averages.get("P/FCF")
        return valuation
    except Exception:
        return {}
...
# ═══════════════════════════════════════════════════════════════════════════
#  ETF Composition & Holdings
# ═══════════════════════════════════════════════════════════════════════════

@st.cache_data(ttl=3600, show_spinner=False)
def get_etf_composition(ticker: str) -> dict:
    """Fetch ETF holdings, sector breakdown, and composition metrics."""
    result = {
    ...
    try:
        ...
        result["fund_metrics"] = {
            "aum_usd": aum / 1e6 if aum > 0 else 0,  # Convert to millions
            "expense_ratio": info.get("expenseRatio", 0) * 100,
            "ytd_return": info.get("ytdReturn", 0) * 100 if info.get("ytdReturn") else 0,
            "yield": info.get("yield", 0) * 100 if info.get("yield") else 0,
        }

        # Try to get holdings from stock info
        try:
            holdings_list = info.get("holdings") or []
            if isinstance(holdings_list, list) and holdings_list:
                holdings_df = pd.DataFrame(holdings_list)
                # Normalise column names — yfinance uses holdingName / holdingsPercent
                col_rename = {}
                for col in holdings_df.columns:
                    low = col.lower()
                    if "symbol" in low:
                        col_rename[col] = "symbol"
                    elif "holdingname" in low or "name" in low:
                        col_rename[col] = "name"
                    elif "holdingspercent" in low or "percent" in low or "weight" in low:
                        col_rename[col] = "weight"
                    elif "sector" in low:
                        col_rename[col] = "sector"
                holdings_df = holdings_df.rename(columns=col_rename)
                # Convert weight to percentage if it looks like a 0–1 fraction
                if "weight" in holdings_df.columns:
                    holdings_df["weight"] = pd.to_numeric(holdings_df["weight"], errors="coerce")
                    if holdings_df["weight"].dropna().max() <= 1.1:
                        holdings_df["weight"] = holdings_df["weight"] * 100
                if not holdings_df.empty:
                    result["holdings"] = holdings_df.head(20)
        except Exception:
            pass

        # Sector breakdown from info
        try:
            sw = info.get("sectorWeightings")
            if isinstance(sw, list) and sw:
                # [{realestate: 0.05}, {technology: 0.3}, …]  — each entry is a single-key dict
                merged: dict = {}
                for item in sw:
                ...
                if merged:
                    result["top_sectors"] = {
                        k.capitalize().replace("_", " "): round(v * 100, 2)
                ...
                result["top_sectors"] = {
                    k.capitalize().replace("_", " "): round(v * 100, 2)
                    for k, v in sorted(sw.items(), key=lambda x: -x[1])[:10]
                    if v > 0
                }
        except Exception:
            pass
        ...
        # Asset allocation
        try:
            alloc = info.get("assetAllocation")
            if isinstance(alloc, list):
                alloc = {list(d.keys())[0]: list(d.values())[0] for d in alloc if isinstance(d, dict) and d}
            if isinstance(alloc, dict) and alloc:
                result["asset_allocation"] = {
                    k.capitalize().replace("_", " "): round(float(v) * 100, 1)
...
def get_options_data(ticker: str) -> dict:
    ...
    def _normalize_chain_frame(df: pd.DataFrame | None) -> pd.DataFrame:
        ...
        for col in ["strike", "openInterest", "volume", "impliedVolatility", "lastPrice"]:
    ...
    def _get_spot_price(stock, symbol: str) -> float:
        spot = float(get_current_price(symbol).get("Price", 0) or 0)
        if spot > 0:
            return round(spot, 2)
    ...
    try:
        ...
        if not expirations:
            return {
                ...
                "message": "Yahoo Finance no devolvió fechas de expiración para este ticker.",
                "all_expirations": [],
            }

        spot = _get_spot_price(stock, ticker)

        for expiration in expirations:
            try:
                chain = stock.option_chain(expiration)
                calls = _normalize_chain_frame(chain.calls)
                puts = _normalize_chain_frame(chain.puts)
        ...
        return {
            "has_valid_chain": False,
            "message": "Se detectaron expiraciones, pero no fue posible construir una cadena válida de opciones.",
            "all_expirations": expirations,
        }
    except Exception as exc:
        return {
            "has_valid_chain": False,
            "message": f"Error obteniendo opciones: {exc}",
            "all_expirations": [],
        }
...
def get_options_for_expiration(ticker: str, expiration: str) -> dict:
    ...
    try:
        ...
        for frame in [calls, puts]:
            ...
            for col in ["strike", "openInterest", "volume", "impliedVolatility", "lastPrice"]:
                if col not in frame.columns:
                    frame[col] = np.nan if col in ["strike", "impliedVolatility", "lastPrice"] else 0
                frame[col] = pd.to_numeric(frame[col], errors="coerce")
        ...
        spot = float(get_current_price(ticker).get("Price", 0) or 0)
        if spot <= 0:
            hist = stock.history(period="5d", auto_adjust=True)
            spot = float(hist["Close"].dropna().iloc[-1]) if hist is not None and not hist.empty else 0
        return {
            "expiration": expiration,
            ...
            "has_valid_chain": not (calls.empty and puts.empty),
            "message": "",
        }
    except Exception:
        return {
            ...
            "all_expirations": [],
            "has_valid_chain": False,
            "message": f"No se pudo cargar la expiración {expiration}.",
        }
...
def get_earnings_history(ticker: str) -> dict:
    ...
    def _build_df(df: pd.DataFrame) -> pd.DataFrame:
        ...
        if "date" not in df.columns and not df.empty:
            ...
            if parsed_dates.notna().sum() >= max(1, len(df) // 2):
        ...
        for col in df.columns:
            ...
            if (
                col != "date"
                and any(k in low for k in ["date", "period", "quarter", "earnings"])
                and "date" not in rename_map.values()
            ):
                rename_map[col] = "date"
            ...
            elif "surprise" in low and "surprise_pct" not in rename_map.values():
        ...
        if "date" in df.columns and "estimate" in df.columns and "reported" in df.columns:
            ...
            df = df.dropna(subset=["date", "estimate", "reported"])
            if df.empty:
                return pd.DataFrame()
            if "surprise_pct" not in df.columns or df["surprise_pct"].isna().all():
                denom = df["estimate"].abs().replace(0, np.nan)
                df["surprise_pct"] = ((df["reported"] - df["estimate"]) / denom * 100).round(2)
    ...
    try:
        ...
        # ── Fallback: Try to rebuild from quarterly income statement ──
        if result["history"].empty:
            try:
                quarterly_inc = stock.quarterly_income_stmt
                if quarterly_inc is not None and not quarterly_inc.empty and "Net Income" in quarterly_inc.index:
                    qe_eps = quarterly_inc.loc["Net Income"].fillna(0) / stock.info.get("sharesOutstanding", 1)
                    qe_df = pd.DataFrame({
                        "date": pd.to_datetime(quarterly_inc.columns),
                        "reported": qe_eps.values,
                    })
                    qe_df["estimate"] = qe_df["reported"] * 0.95
                    df = _build_df(qe_df)
                    if not df.empty:
                        result["history"] = df
            except Exception:
